{
  ...
  outputs = {
    ...
    packages.x86_64-linux.jetbrains-jdk = let
      pkgs = import nixpkgs {
        system = "x86_64-linux";
        config.allowUnfree = true;
      };
    ...
    packages.aarch64-darwin.idea-ultimate = let
      pkgs = import master-nixpkgs {
        system = "aarch64-darwin";
        config.allowUnfree = true;
      };
    in
      pkgs.jetbrains.idea-ultimate.overrideAttrs (final: previous: {
        ...
        src = pkgs.fetchurl {
          ...
          sha256 = "sha256-2K6Trel93TDJH9KoKHY7HJUujCBvBPvbnXnqIgeVWo4=";
        };
        version = "2023.1.2";
      });

    packages.x86_64-linux.idea-ultimate = let
      pkgs = import nixpkgs {
        system = "x86_64-linux";
        config.allowUnfree = true;
      };
    in
      pkgs.jetbrains.idea-ultimate.overrideAttrs (final: previous: {
        ...
        src = pkgs.fetchurl {
          ...
          sha256 = "9675c15bea4b3d0e2b00265f1b4c7c775f4187cfda9b894b4109c90ceb8e3061";
        };
    ...
    packages.aarch64-darwin.code-server = let
      pkgs = import master-nixpkgs {
        system = "aarch64-darwin";
        config.allowUnfree = true;
        config.permittedInsecurePackages = ["nodejs-16.20.1"]; # code-server doesn't support anything else
      };
    in
      (pkgs.code-server.override { nodejs = pkgs.nodejs_16; }).overrideAttrs (final: previous: {
        meta.platforms = previous.meta.platforms ++ ["aarch64-darwin"];

        buildPhase = builtins.replaceStrings [ "darwin-x64" ] [ "darwin-arm64" ] previous.buildPhase;
      });

    formatter.aarch64-darwin =
      (import master-nixpkgs {
        system = "aarch64-darwin";
        config.allowUnfree = true;
      })
      .alejandra;
